...
public class Church extends VillageTComponent
        {
            public Church()
            {
            }

            public Church(AlterniaVillageStart start, int type, Random rand, StructureBoundingBox boundingBox, EnumFacing facing)
            {
                super(start, type);
            ...
            public static Church createPiece(AlterniaVillageStart start, List<StructureComponent> ComponentList, Random rand, int PosX, int PosY, int PosZ, EnumFacing facing, int type)
            {
            	System.out.println(facing+" at "+ PosX+"," + PosZ);
            	
            	
            	
                StructureBoundingBox structureboundingbox = StructureBoundingBox.getComponentToAddBoundingBox(PosX, PosY, PosZ, 0, 0, 0, 5, 12, 9, facing);
                return VillageComponentInterface.canVillageGoDeeper(structureboundingbox) && StructureComponent.findIntersecting(ComponentList, structureboundingbox) == null ? new Church(start, type, rand, structureboundingbox, facing) : null;
            }
